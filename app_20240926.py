...
def load_data():
    ...
    df_postes = pd.read_pickle('LI_REDE_EC_POSTE_concatenado.pkl')
    resultados_fme = resultados_fme[['STATUSIN','CODIGO','COMPRIME','geometry','Nome_Subpasta']]
    df_postes = df_postes[['STATUSIN','CODIGO','geometry','Nome_Subpasta']]
    resultados_fme = resultados_fme.rename(columns={'Nome_Subpasta':'LINHA'})
    df_postes = df_postes.rename(columns={'Nome_Subpasta':'LINHA'})
    ...
    df_postes = df_postes.merge(escopo,how='left',left_on='LINHA',right_on='ALIMENTADOR')
    resultados_fme = resultados_fme.fillna('-')
    df_postes = df_postes.fillna('-')
    
    return resultados_fme, df_postes
...
if authentication_status:
    ...
    with tab2:
        # Carrega os dados
        resultados_fme, df_postes = load_data()
        ...
        tipo_selecionado = st.selectbox("Selecione o tipo de ativo:", ["Trecho", "Poste", "Todos"])
        ...
        regiao_postes_df = df_postes[df_postes['REGIONAL'].isin(regional)]
        ...
        regiao_postes_df['cor'] = df_postes['STATUSIN'].map(status_colors)
        ...
        regiao_postes_df = regiao_postes_df[regiao_postes_df['SUBESTAÇÃO'].isin(subestacao)]
        ...
        regiao_postes_df = regiao_postes_df[regiao_postes_df['LINHA'].isin(linha)]
        ...
        if tipo_selecionado == "Trecho":
            regiao_df['CODIGO'] = regiao_df['CODIGO'].astype(str)
            ...
            regiao_postes_df['CODIGO'] = regiao_postes_df['CODIGO'].astype(str)
        ...
        with st.spinner("Carregando mapa..."):
            ...
            if not regiao_df.empty and not regiao_df['geometry'].isnull().all():
                GeoJson(
                    regiao_df,
                    style_function=lambda feature: {'color': feature['properties']['cor'], 'weight': 2},
                    name='ALIMENTADOR',
                    tooltip=folium.GeoJsonTooltip(fields=['CODIGO'], aliases=['Código:'], localize=True)
                ).add_to(mapa)

            if not regiao_postes_df.empty and not regiao_postes_df['geometry'].isnull().all():
                GeoJson(
                    regiao_postes_df, 
                    style_function=lambda feature: {'color': feature['properties']['cor'], 'radius': 3, 'fill': True}, 
